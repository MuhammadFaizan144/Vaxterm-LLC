                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-10">
                            {services.map((item, index) => (
                                <div
                                    key={index}
                                    className="relative bg-white rounded-3xl p-6 shadow-sm border border-gray-100 -rounded-tr-full hover:shadow-lg transition-all duration-300"
                                >
                                    <div className="absolute top-4 right-4 size-[64px] rounded-full bg-[#F3F4F6] flex items-center justify-center text-sm font-semibold text-gray-500">
                                        {item.id}
                                    </div>
                        ...
                        {/* FAQ */}
                        <section className="w-full max-w-[869px] mx-auto py-9">
                            <div className="space-y-5">
                                {faqs.map((faq, index) => {
                                    ...
                                    return (
                                        <div
                                            key={index}
                                            className={`rounded-[24px] p-5 md:p-6 transition-all duration-300 ${isActive ? "bg-[#031126]" : "bg-[#F3F4F6] border border-[#8081874F]"

                                                }`}
                                        >
                                            <div onClick={() => toggleFAQ(index)} className="flex cursor-pointer items-start gap-5">
                                                {/* Button */}
                                                <button
                                                    
                                                    className={`min-w-[56px] h-[56px] rounded-[16px] text-[28px] flex items-center justify-center transition-all ${isActive
                                                ...
                                                <div>
                                                    <h3
                                                        className={`text-[14px] md:text-[18px] font-semibold leading-tight ${isActive ? "text-white" : "text-[#111827]"
                                                            }`}
                                                    >
                                                        {faq.question}
                                                    </h3>
                                                    {isActive && (
                                                        <p className="text-[#9CA3AF] text-[12px] md:text-[16px] leading-[32px] mt-5 max-w-[700px]">
                                                            {faq.answer}
                                                        </p>
                                                    )}
                                                </div>
                        ...
                        <div className="bg-white rounded-3xl p-6 border border-gray-200 top-6">
                            ...
                            <div className="space-y-4">
                                {solutions.map((item, index) => (
                                    <button
                                        key={index}
                                        className={`w-full flex items-center justify-between rounded-full px-5 py-4 text-[16px] font-medium transition-all duration-300 ${item === "Enterprise Systems"
                                            ? "bg-[#0D7FFB] text-white"
                                            : "bg-white border border-[#DCDCDC] hover:bg-[#E8F1FF] text-[#040B2F]"
                                            }`}
                                    >
                                        {item}
                                        <img src="/images/Solutions/subpages/enterprise/rightarrow.png" alt="" className='w-[18px] h-[13.5px]'/>
                                    </button>
                                ))}
                            </div>
                        </div>
